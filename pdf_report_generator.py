...
class PdfReportGenerator:
    ...
    def generate_report(filename: str, start_br: str, end_br: str, rows: list, glargina_by_date: dict):
        ...
        data_by_date_iso = {}
        for date_iso, meal, carbs, glicemia, lispro, bolus in rows:
            data_by_date_iso.setdefault(date_iso, []).append(
                [meal, f"{carbs:.1f}" if carbs is not None else "",
        ...
        head = ["Refeição", "Carbs (g)", "Glicemia", "Lispro (UI)", "Bolus (UI)"]
        ...
        for date_iso in all_dates_iso:
            ...
            if date_iso in data_by_date_iso:
                ...
                story.append(Table(tbl_data, style=tbl_style))
            ...
            if date_iso in data_by_date_iso:
                for _meal, carbs_str, glic_str, lispro_str, bolus_str in data_by_date_iso[date_iso]:
            ...
            story.append(
                Paragraph(
                    (
                        # ALTERAÇÃO AQUI: Adicionado tags <b> novamente
                        f"Totais do dia – <b>Carbs</b>: {d_tot['carbs']:.1f} g, "
                        f"<b>Glicemia Média</b>: {daily_avg_glicemia:.1f} mg/dL, "
                        f"<b>Lispro</b>: {d_tot['lispro']:.1f} UI, "
        ...
        story.append(
            Paragraph(
                (
                    f"<b>Carboidratos</b>: {grand_totals['carbs']:.1f} g – "
                    f"<b>Glicemia Média</b>: {period_avg_glicemia:.1f} mg/dL – "
                    f"<b>Insulina Lispro total</b>: {grand_totals['lispro']:.1f} UI – "
                    f"<b>Bolus correção total</b>: {grand_totals['bolus']:.1f} UI – "
